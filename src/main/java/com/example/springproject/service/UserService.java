...
public class UserService {
    ...
    public AuthResponse registerPatient(RegisterPatientRequest request, MultipartFile imageFile) {
        ...
        // Save patient (this will also create the User record due to JOINED
        // inheritance)
        patient = patientRepository.save(patient);

        // Generate tokens
        String accessToken = jwtService.generateAccessToken(patient.getEmail(), patient.getId(),
    ...
    public AuthResponse registerVolontaire(RegisterVolontaireRequest request, MultipartFile imageFile) {
        ...
        // Save volontaire (this will also create the User record due to JOINED
        // inheritance)
        volontaire = volontaireRepository.save(volontaire);

        // Generate tokens
        String accessToken = jwtService.generateAccessToken(volontaire.getEmail(), volontaire.getId(),
                volontaire.getRole().name());
    ...
    public AuthResponse registerFamilyMember(RegisterFamilyMemberRequest request, MultipartFile imageFile) {
        ...
        // Save family member (this will also create the User record due to JOINED
        // inheritance)
        familyMember = familyMemberRepository.save(familyMember);

        // Generate tokens
        String accessToken = jwtService.generateAccessToken(familyMember.getEmail(), familyMember.getId(),
                familyMember.getRole().name());
    ...
    public UserResponse getUserById(Long userId) {
        ...
        if (user.getRole() == UserRole.PATIENT) {
            ...
            if (patient != null) {
                ...
                response.setHasCompletedOnboarding(patient.getHasCompletedOnboarding());
                response.setProfileCompletionScore(patient.getProfileCompletionScore());
    ...
    public boolean verifyOtp(VerifyOtpRequest request) {
        Otp otp = otpRepository.findByEmailAndCodeAndUsedFalseAndExpiresAtAfter(
                request.getEmail(),
                request.getOtpCode(),
                LocalDateTime.now()).orElseThrow(() -> new RuntimeException("Invalid or expired OTP code"));
    ...
    public AuthResponse verifyRegistrationOtp(String email, String otpCode, String userRole, MultipartFile imageFile) {
        System.out.println("Verifying OTP for: " + email + ", Code: " + otpCode + ", Role: " + userRole + ", Time: "
                + LocalDateTime.now());
        // Verify OTP
        Otp otp = otpRepository.findByEmailAndCodeAndUsedFalseAndExpiresAtAfter(
                email,
